...
=back
    ...
    my $logged = Cater::Log->admin_log( \%log_data );
...
=back
    ...
    my $diffs = Cater::Log->find_changes_in_data( old_data => $old, new_data => $new );
...
=back
    ...
    my $logs = Cater::Log->get_admin_logs( page => 1, per_page => 50 );
